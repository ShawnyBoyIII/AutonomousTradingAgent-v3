...
(function () {
  ...
  function setInnerHTMLAndRestoreFocus(container, htmlContent) {
    if (!container) return;

    // Capture currently focused element if it's inside the container
    ...
    if (activeId) {
      ...
      const els = container.getElementsByClassName(activeClass);
      if (els.length > 0) {
        els[0].focus();
      }
    }
  ...
  function renderPositions(positions) {
    ...
    if (!positions.length) {
      setInnerHTMLAndRestoreFocus(container, '<div class="empty">No open positions</div>');
      return;
    }
    ...
    setInnerHTMLAndRestoreFocus(container, `
  ...
  function renderClosedTrades(payload, options = {}) {
    ...
    if (!trades.length) {
      ...
      setInnerHTMLAndRestoreFocus(container, '<div class="empty">No closed trades yet</div>');
      return;
    }
    ...
    if (table && focusExpanded) table.focus();
  ...
  function renderAlerts(data) {
    ...
    if (!alerts.length) {
      setInnerHTMLAndRestoreFocus(container, '<div class="empty">All systems nominal</div>');
      return;
    }

    setInnerHTMLAndRestoreFocus(container, alerts.map((a) => {
  ...
  function renderHealth(data) {
    ...
    if (!checks.length) {
      setInnerHTMLAndRestoreFocus(container, '<div class="empty">No checks reported</div>');
      return;
    }

    setInnerHTMLAndRestoreFocus(container, checks.map((c) => {
  ...
  function renderTrades(data) {
    ...
    if (!trades.length) {
      setInnerHTMLAndRestoreFocus(container, '<div class="empty">No recent trades</div>');
      return;
    }
