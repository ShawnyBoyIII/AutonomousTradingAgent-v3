#!/usr/bin/env python3
"""Multi-symbol training: SPY + QQQ + AAPL + NVDA in a single environment."""
import sys
import json
...
TIMESTEPS = 300_000
OUTPUT_DIR = Path("state/rl_logs/multisymbol")
...
def train_one(seed: int) -> int:
    tag = f"seed_{seed}"
    print(f"\n[Seed {seed}] Training {SYMBOLS}...", flush=True)
...
def main() -> int:
    print(f"Multi-symbol training: {SYMBOLS}, {len([123, 789])} seeds, {TIMESTEPS:,} timesteps each", flush=True)
    seeds = [123, 789]
    for seed in seeds:
        try:
            train_one(seed)
            ...
            import traceback
            traceback.print_exc()
    print("\nMulti-symbol training complete.", flush=True)
