...
## 2026-08-03 - Focus management and state persistence on DOM removal
**Learning:** Found that the "Show X more" button in the closed trades list was removed from the DOM upon click, causing keyboard focus to be lost (dropped to document body). Also, the expand action only appended HTML statelessly; if the dashboard polled for live updates via SSE, the table would collapse back to its truncated limit because the state wasn't tracked globally.
**Action:** When a button action removes the button itself from the DOM, explicitly check `document.activeElement === button` *before* removal, and if true, shift focus (`tabindex="-1"` and `.focus()`) to the newly revealed content container so keyboard users stay oriented. Furthermore, bind component visibility states (like expanded/collapsed lists) to a centralized UI state object so background live updates re-render the intended view rather than resetting to default.
